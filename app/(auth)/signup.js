...
const normalizeWorkKey = (k) => (k?.startsWith("/works/") ? k : k ? `/works/${k}` : null);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      <View style={styles.headerRow}>
        <TouchableOpacity onPress={step === 0 ? goToLogin : back} style={styles.backBtn}>
          <Text style={{ fontWeight: "900" }}>{step === 0 ? "Login" : "Back"}</Text>
        </TouchableOpacity>
      ...
      {step === 0 && (
        <Surface>
          ...
          <TextInput style={styles.input} value={username} onChangeText={setUsername} placeholder="janedoe" autoCapitalize="none" />
      ...
      {step === 1 && (
        <>
          ...
          <Surface>
            <View style={styles.chipsWrap}>
              {(themeQuery.trim() ? filteredGenres : filteredGenres.slice(0, 60)).map((g) => (
                <Chip key={g} label={g} selected={themes.includes(g)} onPress={() => toggleTheme(g)} />
              ))}
            </View>
            <Text style={styles.hint}>Tip: use the search box to find a theme faster.</Text>
          </Surface>
        </>
      )}
          ...
          <Surface style={{ marginBottom: 12 }}>
            ...
            <View style={{ flexDirection: "row", gap: 10 }}>
              ...
              <View style={{ width: 110 }}>
                <PrimaryButton title="Search" onPress={fetchFavResults} disabled={favLoading || !favQuery.trim()} />
              </View>
            </View>
                ...
                {favResults.map((b) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text style={{ fontWeight: "900", color: selected ? Colors.success : Colors.muted }}>
                        {selected ? "✓" : "+"}
                      </Text>
                    </TouchableOpacity>
                  );
      ...
      <View style={styles.footerRow}>
        {step < 3 ? (
          ...
          <PrimaryButton title="Back to login" variant="secondary" onPress={goToLogin} />
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  stepTitle: { fontSize: 16, fontWeight: "800", color: Colors.muted, marginBottom: 10 },
  ...
  label: { fontSize: 12, fontWeight: "800", color: Colors.muted, marginTop: 10, marginBottom: 6 },
  input: {
    height: 44,
    borderRadius: 12,
    backgroundColor: Colors.surface,
  ...
  resultRowSelected: { borderColor: "rgba(22,163,74,0.35)", backgroundColor: "rgba(22,163,74,0.06)" },
